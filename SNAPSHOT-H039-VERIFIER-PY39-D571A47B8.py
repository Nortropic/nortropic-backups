"""Fixed 39-file frozen verifier py39 D qualification checkpoint, no code execution.
...
LIST=HERE/'h039-verifier-py39-D-qualified-selection.json'
ARCHIVE=BASE/'external/h039-verifier-py39-D571a47b8-qualified-20260912.tar.gz'
MANIFEST=HERE/'h039-verifier-py39-D-qualified-inspection.json'
DOWNLOAD=HERE/'h039-verifier-py39-D571a47b8-qualified-20260912.download.tar.gz'
RECEIPT=HERE/'h039-verifier-py39-D-qualified-restore.json'
def sha(p):
...
if sys.argv[1]=='prepare':
 assert sha(LIST)=='0a155a609c5c0e231ef5f47ece03ace2c2c50d746746de3ef58aa6304c10659e'
 items=json.loads(LIST.read_bytes());assert items['count']==38
 selected={Path(x['path']):x['sha256'] for x in items['rows']};selected[LIST]=sha(LIST);assert len(selected)==39
 ...
 expected={str(p.relative_to('/')):p for p in selected};assert len(expected)==39
 ...
 target=Path(tempfile.mkdtemp(prefix='nortropic-h039-verifier-py39-D-qualified-restore-',dir='/private/tmp'))
 ...
 rows=[x for x in m['files'] if 'source' in x];assert len(rows)==39
