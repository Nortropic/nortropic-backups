"""Fixed 31-file frozen verifier C qualification checkpoint, no code execution.
...
LIST=HERE/'h039-verifier-C-qualified-selection.json'
ARCHIVE=BASE/'external/h039-verifier-C83f75968-qualified-20260912.tar.gz'
MANIFEST=HERE/'h039-verifier-C-qualified-inspection.json'
DOWNLOAD=HERE/'h039-verifier-C83f75968-qualified-20260912.download.tar.gz'
RECEIPT=HERE/'h039-verifier-C-qualified-restore.json'
def sha(p):
...
if sys.argv[1]=='prepare':
 assert sha(LIST)=='da6e11475e46c2796ca07dbbd003c8d3e91f3bce01990011390139f6199db050'
 items=json.loads(LIST.read_bytes());assert items['count']==30
 selected={Path(x['path']):x['sha256'] for x in items['rows']};selected[LIST]=sha(LIST);assert len(selected)==31
 ...
 expected={str(p.relative_to('/')):p for p in selected};assert len(expected)==31
 ...
 save(MANIFEST,dict(status='LOCAL_INSPECTED_NOT_UPLOADED',archive_sha256=sha(ARCHIVE),archive_bytes=ARCHIVE.stat().st_size,files=rows,findings=findings,logical_files=31,source_unchanged=True,command=cmd,exit=0,scope='Six frozen verifier C83f75968 files, held intermediate and final controls, preparation and actual qualification captures, independent review, continuity and bundle backup records/method plus selection; not original Git administration or whole-system restoration',runtime_credit=False))
 print(json.dumps(dict(files=len(rows),logical_files=31,bytes=ARCHIVE.stat().st_size,sha256=sha(ARCHIVE),findings=findings)))
else:
 ...
 target=Path(tempfile.mkdtemp(prefix='nortropic-h039-verifier-C-qualified-restore-',dir='/private/tmp'))
 ...
 rows=[x for x in m['files'] if 'source' in x];assert len(rows)==31
 ...
 save(RECEIPT,dict(time_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),status='DOWNLOADED_31_VERIFIER_C_FILES_RESTORED_VERIFIED',inspection_sha256=sha(MANIFEST),archive_sha256=m['archive_sha256'],archive_bytes=m['archive_bytes'],restored=str(target),logical_files=31,checked=['file_membership','bytes','size','mode','uid','gid','mtime_ns','xattrs','single_link_independent_inode'],directory_metadata_restored=False,original_inode_recreated=False,project_code_executed=False,runtime_credit=False,full_final_fixture_restored=False))
 print('31 actual downloaded selected files restored, no code execution')
