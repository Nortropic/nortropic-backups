"""Fixed 12-file held verifier WIP checkpoint, no code execution.
...
LIST=HERE/'h039-verifier-draft-selection.json'
ARCHIVE=BASE/'external/h039-verifier-draft-P005366dd-20260912.tar.gz'
MANIFEST=HERE/'h039-verifier-draft-inspection.json'
DOWNLOAD=HERE/'h039-verifier-draft-P005366dd-20260912.download.tar.gz'
RECEIPT=HERE/'h039-verifier-draft-restore.json'
def sha(p):
...
if sys.argv[1]=='prepare':
 assert sha(LIST)=='36c69f7ced73350ecd5d5ac8005ec237c8f619335792afe80e3981a63e3aedfb'
 items=json.loads(LIST.read_bytes());assert items['count']==11
 selected={Path(x['path']):x['sha256'] for x in items['rows']};selected[LIST]=sha(LIST);assert len(selected)==12
 ...
 expected={str(p.relative_to('/')):p for p in selected};assert len(expected)==12
 ...
 save(MANIFEST,dict(status='LOCAL_INSPECTED_NOT_UPLOADED',archive_sha256=sha(ARCHIVE),archive_bytes=ARCHIVE.stat().st_size,files=rows,findings=findings,logical_files=12,source_unchanged=True,command=cmd,exit=0,scope='Six held unfrozen verifier-contract WIP files on exact P005366dd, three test records, root preservation record and continuity plus selection; not original Git administration or whole-system restoration',runtime_credit=False))
 print(json.dumps(dict(files=len(rows),logical_files=12,bytes=ARCHIVE.stat().st_size,sha256=sha(ARCHIVE),findings=findings)))
else:
 ...
 target=Path(tempfile.mkdtemp(prefix='nortropic-h039-verifier-draft-restore-',dir='/private/tmp'))
 ...
 rows=[x for x in m['files'] if 'source' in x];assert len(rows)==12
