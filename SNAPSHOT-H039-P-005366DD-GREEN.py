"""Fixed 96-file P corrected-final/review checkpoint, no code execution.
...
LIST=HERE/'h039-P-005366dd-green-selection.json'
ARCHIVE=BASE/'external/h039-P-005366dd-corrected-final-green-20260912.tar.gz'
MANIFEST=HERE/'h039-P-005366dd-green-inspection.json'
DOWNLOAD=HERE/'h039-P-005366dd-corrected-final-green-20260912.download.tar.gz'
RECEIPT=HERE/'h039-P-005366dd-green-restore.json'
def sha(p):
...
if sys.argv[1]=='prepare':
 assert sha(LIST)=='607f60058bf132cad9b625025963feb629c08e230eb65161a5ece29faa791911'
 items=json.loads(LIST.read_bytes());assert items['count']==95
 selected={Path(x['path']):x['sha256'] for x in items['rows']};selected[LIST]=sha(LIST);assert len(selected)==96
 ...
 expected={str(p.relative_to('/')):p for p in selected};assert len(expected)==96
 ...
 save(MANIFEST,dict(status='LOCAL_INSPECTED_NOT_UPLOADED',archive_sha256=sha(ARCHIVE),archive_bytes=ARCHIVE.stat().st_size,files=rows,findings=findings,logical_files=96,source_unchanged=True,command=cmd,exit=0,scope='Exact R2 preparation methods/results, corrected final captures/result/full subject snapshot, independent postreview, fresh two build lanes and current continuity snapshot; not full final fixture, original repository administration or a whole-system restoration',runtime_credit=False))
 print(json.dumps(dict(files=len(rows),logical_files=96,bytes=ARCHIVE.stat().st_size,sha256=sha(ARCHIVE),findings=findings)))
else:
 ...
 target=Path(tempfile.mkdtemp(prefix='nortropic-h039-P-green-restore-',dir='/private/tmp'))
 ...
 rows=[x for x in m['files'] if 'source' in x];assert len(rows)==96
 ...
 save(RECEIPT,dict(time_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),status='DOWNLOADED_96_P_GREEN_FILES_RESTORED_VERIFIED',inspection_sha256=sha(MANIFEST),archive_sha256=m['archive_sha256'],archive_bytes=m['archive_bytes'],restored=str(target),logical_files=96,checked=['file_membership','bytes','size','mode','uid','gid','mtime_ns','xattrs','single_link_independent_inode'],directory_metadata_restored=False,original_inode_recreated=False,project_code_executed=False,runtime_credit=False,full_final_fixture_restored=False))
 print('96 actual downloaded selected files restored, no code execution')
